...
{
private:
	std::vector<T*>* map = nullptr;
	std::vector<T*>* mapEnd = nullptr;

	SCALAR x, y, maxSearchWidth;
	int iWidth, iHeight;
...
private:
	template<typename T> static void erase_unordered(std::vector<T>& v, size_t index)
	{
		v[index] = v.back();
		v.pop_back();
	}
	...
	std::pair<int, int> ConvertFromWorldToGrid(SCALAR& worldX, SCALAR& worldY)
	{
		return std::make_pair<int, int>(
			(worldX - this->x) / maxSearchWidth,
			(worldY - this->y) / maxSearchWidth);
	}
...
public:
	...
	void Update(std::vector<T>& boids)
	{
		for (Boid& boid : boids)
		{
